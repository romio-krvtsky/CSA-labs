...
.data
# asciiz директива используетс€ д€л размещени€ строк
# asciiz автоматически добавит нулевой символ в конец строки
# FIXME исправьте строки, чтобы они выдавали корректные отладочные значени€ 
...
# FIXME –азместите значени€ из вашего варианта в этом массиве 
...
main:
    ...
    la a0, case1 
    # ¬ыводим строку по адресу case1
    jal print_str 
    # «агружаем первый аргумент функции f в a0
    # FIXME ѕодставьте первый аргумент case1
    li a0, -3 
    # загружаем второй аргумент функции f в a1
    # `output` -- это указатель на массив, который содержит возможные выходные значени€ f
    la a1, output
    # выполн€ем f(case1)
    jal f     
    ...
    # значение уже находитс€ в a0, не требуетс€ перемещений
    jal print_int
    # print a new line
    jal print_newline
...
# a0 значение дл€ которого мы хотим вычислить функцию f
